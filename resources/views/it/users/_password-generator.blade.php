...
<div x-data="passwordGenerator()" class="space-y-2">
    ...
    <div class="flex gap-2">
        ...
        <button type="button" @click="visible = !visible"
                class="px-3 py-2 text-xs font-medium border border-gray-300 rounded-md text-gray-600 hover:bg-gray-50"
                x-text="visible ? 'Sembunyikan' : 'Lihat'"></button>
        <button type="button" @click="copy()" :disabled="!password"
                class="px-3 py-2 text-xs font-medium border border-gray-300 rounded-md text-gray-600 hover:bg-gray-50 disabled:opacity-40"
                x-text="copied ? 'Tersalin!' : 'Copy'"></button>
    </div>
    <button type="button" @click="generate()"
            class="px-3 py-1.5 text-xs font-medium rounded-md bg-indigo-50 text-indigo-700 hover:bg-indigo-100">
