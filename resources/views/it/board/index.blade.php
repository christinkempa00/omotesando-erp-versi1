<x-app-layout sidebar="it">
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('it.labels.index') }}" class="text-sm text-accent hover:text-accent-dark font-medium">
                Kelola Label
            </a>
        </div>
    </x-slot>
                            ...
                            <template x-for="task in column.tasks" :key="task.id">
                                <li :data-task-id="task.id" @click="openTask(task)"
                                    ...
                                    <div class="flex items-center justify-between text-xs">
                                        ...
                                        <span class="w-6 h-6 rounded-full bg-accent text-white flex items-center justify-center text-[10px] font-semibold"
                        ...
                        <div class="mt-2" x-data="{ adding: false, title: '' }">
                            ...
                            <form x-show="adding" x-cloak
                                ...
                                <textarea x-ref="newTaskInput" x-model="title" rows="2" placeholder="Judul task..."
                                          ...
                                          class="w-full text-xs rounded-md border-gray-300 focus:border-accent focus:ring-accent"></textarea>
                                <div class="flex items-center gap-2">
                                    <button type="submit" class="px-2.5 py-1 text-xs font-medium rounded-md bg-accent text-white hover:opacity-90 transition">Tambah</button>
                    ...
                    <div class="p-6 space-y-5">
                        <div class="flex items-start justify-between gap-3">
                            <input type="text" x-model="activeTask.title" @change="saveField('title', activeTask.title)"
                                   class="flex-1 text-lg font-semibold text-gray-800 border-0 border-b border-transparent hover:border-gray-200 focus:border-accent focus:ring-0 px-0">
                        ...
                        <textarea x-model="activeTask.description" @change="saveField('description', activeTask.description)"
                                  ...
                                  class="w-full text-sm rounded-md border-gray-200 focus:border-accent focus:ring-accent"></textarea>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <div>
                                ...
                                <select x-model="activeTask.priority" @change="saveField('priority', activeTask.priority)"
                                        class="w-full text-xs rounded-md border-gray-300 focus:border-accent focus:ring-accent">
                        ...
                        <div>
                            ...
                            <form @submit.prevent="addChecklistItem($refs.checklistInput.value); $refs.checklistInput.value = ''" class="flex gap-2">
                                <input type="text" x-ref="checklistInput" placeholder="Tambah item checklist..."
                                       class="flex-1 text-xs rounded-md border-gray-300 focus:border-accent focus:ring-accent">
