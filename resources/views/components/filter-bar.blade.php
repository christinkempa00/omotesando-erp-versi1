...
<form method="GET" action="{{ $action ?? url()->current() }}" x-data="{ open: false }" class="flex items-center gap-3">
    @if ($search)
        <div class="relative flex-1 max-w-xs">
            <input type="text" name="{{ $searchName }}" value="{{ $searchValue }}" placeholder="{{ $searchPlaceholder }}"
                   class="w-full rounded-full border-gray-300 pl-4 pr-9 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
    ...
    <div class="relative shrink-0">
        <button type="button" @click="open = !open"
                class="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-full text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
        ...
        <div x-show="open" @click.outside="open = false" x-cloak
            ...
            <div class="flex items-center gap-2 px-5 py-4 border-t border-gray-100">
                <button type="submit" class="flex-1 px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-md hover:bg-gray-800">
                    Terapkan Filter
                </button>
